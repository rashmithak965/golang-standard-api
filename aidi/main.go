...
func basicAuth(username, password string) string {
  ...
  func main() {
	...
	//getconfigmap
	router.HandleFunc("/getConfigMap", controllers.GetConfigMap).Methods("POST")

	//list
	router.HandleFunc("/listdep/{cluster}", controllers.ListDeployments).Methods("GET")
	router.HandleFunc("/listdep", controllers.ListAllDeployemnts).Methods("GET")
	router.HandleFunc("/listcluster/{projectname}/{serviceaccount", controllers.ListCluster).Methods("GET")
	router.HandleFunc("/listsvcgrp/{clustername}", controllers.ListSvcGrps).Methods("GET")

	config_maps.CloneConfigs()

	schedulerSwitch :=os.Getenv("SCHEDULER_SWITCH")

	if schedulerSwitch == "ON" {
		gocron.Every(15).Seconds().Do(schedulers.TriggerBuild)
		gocron.Every(15).Seconds().Do(schedulers.TriggerAutoTriggerBuild)
		gocron.Every(120).Seconds().Do(config_maps.CloneConfigs)
		gocron.Every(300).Seconds().Do(schedulers.TriggerUpdateDeploymentStatus)
		gocron.Start()
	}
